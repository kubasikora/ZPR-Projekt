...
{  
    //  Py_Initialize();
    // std::unique_ptr<std::vector<Measurement>> measurements = std::make_unique<std::vector<Measurement>>();
    ...
    // BOOST_CHECK_EQUAL(service.resample(std::move(measurements)).size(),100 );
...
//     BOOST_CHECK_EQUAL(service.resample(std::move(measurements)).size(),size );
